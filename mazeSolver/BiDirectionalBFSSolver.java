...
public class BiDirectionalBFSSolver implements MazeSolver {
    ...
    /**
     * This solver performs BFS searches starting at both the entrance and exits. When the two BFS fronts
     first meet, the path from the entrance to the point they meet, and the path from the exit to the
     meeting point forms the two halves of a shortest path (in terms of cell visited) from entrance to exit.
     Combine these paths to get the nal path solution.
		...
		do {
			...
			if (!solved && exitCell != null)
			{
				if(!isVisited(exitCell.r, exitCell.c))
				{
					...
					cellsExplored++;
					entranceQueue.addAll(findPossibleMoveOptions(exitCell));
				}
				else
					solved = true;
			}
